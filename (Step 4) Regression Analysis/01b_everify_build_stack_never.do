...
* Build never-treated-only stack:
* - treated cohorts exclude Alabama and Indiana
* - controls are states never treated by restrictive E-Verify
...
egen ever_restrictive_everify = max(is_inc), by(statefip)
...
preserve
    ...
    save "Data/everify_treat_lookup_never.dta", replace
restore

merge m:1 statefip using "Data/everify_treat_lookup_never.dta", nogenerate


keep state statefip year perwt treat_year e_verify is_inc ever_restrictive_everify ///
...
save "Data/everify_clean_master_never.dta", replace

tempfile stack1 stack2 stack3 stack4

* 2010 cohort
use "Data/everify_clean_master_never.dta", clear
gen ref_year = year - 2010
keep if ref_year >= -2 & ref_year <= 3
gen in_cohort = (treat_year == 2010)
gen pot_control = (ever_restrictive_everify == 0)
...
save "Data/everify_stack_never.dta", replace
